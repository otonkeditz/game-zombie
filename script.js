(function setup(window) {
  ...
  Array.prototype.remove = function (x) {
    let a = [];
    for (let i in this) if (i != x) a.push(this[i]);
    return a;
  };
...
(function play(gameover) {
  ...
  var player = {
    ...
    say: function (s) {
      this.message = s;
      if (!this.speak) this.speak = true;
    },
    softcore: ["Revolver", "Shotgun", "Machine Gun", "Sniper Rifle"],
    hardcore: ["Minigun", "Mega Shotgun", "Laser Gun", "Ring of Fire"],
    weapon: "Revolver",
    ...
    fireRound: function (n) {
      ...
      if (!this.out)
        this.bullets.push({
          x:
            this.weapon == "Ring of Fire"
              ? this.x
              : this.x + Math.cos(a) * 2 * this.s,
          y:
            this.weapon == "Ring of Fire"
              ? this.y
              : this.y + Math.sin(a) * 2 * this.s,
          hit: false,
          hits: 0,
          px: Math.cos(a + n),
          py: Math.sin(a + n),
  ...
  for (let i = 0; i < 10; i++) zombies.push(new Zombie());
  var shadow = {
    apply: function () {
      ctx.shadowBlur = 30;
  ...
  var grass = [];
  for (let i = 0; i < 100; i++) grass.push(new Grass());
  var frames = 0;
  (function update() {
    ctx.beginPath();
    ...
    if (!gameover) {
      for (let i in player.bullets) {
        ...
        for (let x in zombies) {
          ...
          if (
            ...
            p.hits++;
            if (p.hits == player.hits) p.hit = true;
            ...
            if (z.special) {
              player.weapon = (function () {
                let a = [];
                let w =
                  zombies.length < player.hardcoreMode
                    ? player.softcore
                    : player.hardcore;
                for (let n = 0; n < w.length; n++)
                  if (player.weapon != w[n]) a.push(w[n]);
                return rand(a);
              })();
              player.applyWeaponProperties();
              player.lives++;
      ...
      if (frames == player.mark + 500) player.speak = false;
      p = player;
      ...
      for (let x = -1; x <= 1; x += 2) {
        ...
        ctx.strokeStyle =
          p.weapon == "Ring of Fire" && !p.out ? "red" : "black";
        ctx.fillStyle = "rgb(150, 100, 50)";
        ctx.arc(
          p.x +
            p.s * Math.cos(a + (x * 30 * Math.PI) / 180) +
            x * p.a * Math.cos(a),
          p.y +
            p.s * Math.sin(a + (x * 30 * Math.PI) / 180) +
            x * p.a * Math.sin(a),
          p.s / 3,
          0,
          2 * Math.PI
      ...
      if (p.showGun) {
        ...
        ctx.moveTo(p.x, p.y);
        ctx.lineTo(
          p.x + Math.cos(a) * p.barrelSize * p.s,
      ...
      for (let x = -1; x <= 1; x += 2) {
        ...
        ctx.strokeStyle = "black";
        ctx.arc(
          p.x + (Math.cos(a + (x * 35 * Math.PI) / 180) * p.s) / 2,
        ...
        ctx.arc(
          p.x +
            (Math.cos(a + (x * 35 * Math.PI) / 180) * p.s) / 2 +
            (Math.cos(a) * p.s) / 8,
          p.y +
            (Math.sin(a + (x * 35 * Math.PI) / 180) * p.s) / 2 +
            (Math.sin(a) * p.s) / 8,
          p.s / 8,
          0,
          2 * Math.PI
      ...
      if (p.fire && frames % p.fireRate === 0)
        switch (p.weapon) {
          case "Laser Gun":
            for (let i = -45; i <= 45; i += 45) p.fireRound(i / 10);
            break;
          default:
            p.fireRound();
        }
    ...
    for (let i in zombies) {
      ...
      for (let x = -1; x <= 1; x += 2) {
        ...
        ctx.arc(
          p.x +
            Math.cos(a + (x * 30 * Math.PI) / 180) * p.s +
            x * p.a * Math.cos(a),
          p.y +
            Math.sin(a + (x * 30 * Math.PI) / 180) * p.s +
            x * p.a * Math.sin(a),
          p.s / 3,
          0,
          2 * Math.PI
    ...
    if (gameover) {
      ...
      ctx.fillText(
        "YOU DIED! " +
          (player.zombiesKilled < 1
            ? "No"
            : player.zombiesKilled.toLocaleString()) +
          " Zombie" +
      ...
      ctx.fillText(
        (player.magSize - player.bullets.length) / player.spray +
          "/" +
          player.magSize / player.spray,
        0,
        40
      );
      ctx.beginPath();
    ...
    .on("mouseup", function () {
      switch (player.weapon) {
        ...
        case "Laser Gun":
          ...
          break;
        case "Shotgun":
          for (let i = -1; i <= 1; i++) player.fireRound(i / 10);
          break;
        case "Mega Shotgun":
          for (let i = -2; i <= 2; i++) player.fireRound(i / 10);
          break;
        case "Ring of Fire":
          for (let i = -90; i <= 90; i += 5) player.fireRound(i);
      }
      player.fire = false;
    })
  ...
  window
    .on("resize", function () {
      ...
      for (let i in grass) grass[i] = new Grass();
    })
    .on("keydown", move)
    .on("keyup", function (e) {
      switch (e.which || e.keyCode) {
